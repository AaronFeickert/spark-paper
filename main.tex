...
\textbf{Addresses}: Users generate keys and addresses that enable transactions.
...
\item $\func{CreateAddress}$: This algorithm outputs a public address, incoming view key, full view key, and spend key.
...
\item $\func{Identify}$: This algorithm takes as input a coin and an incoming view key, and outputs the coin value and memo if the coin is controlled by the key.
\item $\func{Recover}$: This algorithm takes as input a coin and a full view key, and outputs the coin value, memo, serial number, and tag if the coin is controlled by the key.
...
Further, we describe modified algorithms suitable for multisignature operations in Appendix \ref{app:multisig}.
...
\item Compute $Q_1 = \comm(s_1, 0, 0)$ and $Q_2 = \comm(s_2, r, 0)$.
...
\item Sample $k \in \F$.
\item Compute the recovery key $K = \comm(k, 0, 0)$.
\item Compute the serial number commitment $$S = \comm(\hash_{\text{ser}}(kQ_1,Q_1,Q_2), 0, 0) + Q_2.$$
\item Generate the value commitment $C = \com(v, \hash_{\text{val}}(kQ_1))$.
\item If $b=0$, generate a range proof $$\Pi_{\text{rp}} = \func{RangeProve}(pp_{\text{rp}},C;(v,\hash_{\text{val}}(kQ_1))).$$
\item Generate a symmetric encryption key $k_{\text{aead}} = \func{AEADKeyGen}(kQ_1)$; encrypt the value $$\overline{v} = \func{AEADEncrypt}(k_{\text{aead}},\texttt{val},v)$$ (if $b=0$) and memo $$\overline{m} = \func{AEADEncrypt}(k_{\text{aead}},\texttt{memo},m).$$
...
\item Generate a value representation proof on the value commitment: $$\Pi_{\text{bal}} = \func{RepProve}(pp_{\text{rep}},H,C - \com(v,0); \hash_{\text{val}}(kQ_1))$$
...
\textbf{Inputs}: Security parameter $\lambda$, public parameters $pp$, incoming view key $\addr_{\text{in}}$, public address $\addr_{\text{pk}}$, coin $\func{Coin}$.
...
\item If $\comm(\hash_{\text{ser}}(s_1 K,Q_1,Q_2), 0, 0) + Q_2 \neq S$, return failure.
...
\textbf{Inputs}: Security parameter $\lambda$, public parameters $pp$, full view key $\addr_{\text{full}}$, public address $\addr_{\text{pk}}$, coin $\func{Coin}$.
...
\item Arrange the incoming view key as $\addr_{\text{in}} = s_1$.
\item Obtain the value and memo $(v, m) = \func{Identify}(\addr_{\text{in}},\addr_{\text{pk}},\func{Coin})$ from the coin, and return failure if this operation fails.
...
\begin{enumerate}
    ...
    \left\{ S_u', C_u', T_u, (\Pi_{\text{par}})_u \right\}_{u=0}^{w-1}, \Pi_{\text{chaum}}, \Pi_{\text{bal}} )
    \end{multline*}
\end{enumerate}
...
The key and proof structures in Spark permit flexible and useful functionality relating to transaction scanning, generation, and disclosure.

The incoming view key is used in $\func{Identify}$ operations to determine when a coin is directed to the associated public address, and to determine the coin's value and associated memo data.
...
The full view key is used in $\func{Recover}$ operations to additionally compute the serial number and tag for coins directed to the associated public address.
...
In the case where a public address is used in threshold multisignature operations (described in Appendix \ref{app:multisig}), a cosigner may wish to know if or when another cohort of cosigners has produced a transaction spending funds.
...
We describe a method for signing groups to perform the $\func{CreateAddress}$ and $\func{Spend}$ algorithms to produce multisignature keys and spend transactions indistinguishable from others.
...
To produce key and address components, each player $1 \leq \alpha \leq \nu$ engages in the following two-round key generation process:
\begin{enumerate}
    ...
    \end{align*}
\end{enumerate}
Using the tuple $(s_1,s_2,D)$, any player can additionally compute the address components $Q_1 = \comm(s_1,0,0)$ and $Q_2 = \comm(s_2,0,0) + D$.
...
Because all players possess the aggregate full view key corresponding to the public address, any player can use it to construct all transaction components except the modified Chaum-Pedersen proof.
...
For $\func{CreateAddress}$ queries, $\mathcal{C}$ runs the $\func{CreateAddress}$ protocol algorithm and returns the public address $\addr_{pk}$ to $\mathcal{A}$.
...
\begin{itemize}
    ...
    \item $v_{\func{ADDR} \to \mathcal{A}}$ is the total value of coins received by $\mathcal{A}$ from addresses in $\func{ADDR}$; and
    \item $v_{\mathcal{A} \to \func{ADDR}}$ is the total value of coins sent by the adversary to the addresses in $\func{ADDR}$.
\end{itemize}
...
That is, $\mathcal{A}$ produces a spend transaction $\text{tx}_{\text{spend}}$ by an $\func{Insert}$ query that is valid on the augmented ledger $(L, \vec{a})$ and consumes a coin corresponding to a coin serial number commitment $S$ that can be recovered by a public address $(Q_1, Q_2) \in \func{ADDR}$.
...
Now let $(s_1,s_2,r)$ be the spend key corresponding to the address $(Q_1,Q_2)$.
...
Further, the coin $S = \comm(s,r,0)$ consumed in $\text{tx}$ was generated such that $r$ is a spend key component for an address $(Q_1,Q_2)$ not controlled by $\mathcal{A}$.
Since $\mathcal{A}$ does not control this address, it cannot produce $r$ without extracting from $S$ or from $(Q_1,Q_2)$, which implies a non-negligible discrete logarithm advantage, a contradiction.
\end{proof}
...
\begin{itemize}
    ...
    \item Answering $\func{CreateAddress}$ queries: In this case the challenger replaces the public address components $Q_1$ and $Q_2$ with random strings of the appropriate lengths, producing $\addr_{pk}$ that is returned to $\mathcal{A}$.
    ...
    \begin{enumerate}
        ...
        \item Computes the binding hash $\mu$ as defined and simulates the modified Chaum-Pedersen proof $\Pi_{\text{chaum}}$ on the statement $(\{S_u', T_u\}_{u=0}^{w-1})$.
        \item Assembles the transaction and adds it to the ledger as appropriate.
    \end{enumerate}
\end{itemize}
...
By definition, all answers sent to $\mathcal{A}$ in $\mathcal{D}^{\text{sim}}$ are computed independently of the bit $b$, so $\func{Adv}^{\mathcal{D}^{\text{sim}}} = 0$. We will prove that $\mathcal{A}$'s advantage in the real L-IND experiment $\mathcal{D}^{\text{real}}$ is at most negligibly different than $\mathcal{A}$'s advantage in $\mathcal{D}^{\text{sim}}$.
